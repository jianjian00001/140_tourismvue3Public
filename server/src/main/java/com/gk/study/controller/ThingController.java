...
import com.gk.study.entity.ThingVo;
...
public class ThingController {
    ...
    public APIResponse list(String keyword, String sort, String c, String tag,String admin){
        List<Thing> list =  service.getThingList(keyword, sort, c, tag,admin);
    ...
    public APIResponse detail(String id){
        ...
        //访问量加一
        String pv = thing.getPv();
        Integer integer = Integer.valueOf(pv);
        integer++;
        thing.setPv(String.valueOf(integer));
        service.updateThingInfo(thing);
        ThingVo thingVo = new ThingVo();
        BeanUtils.copyProperties(thing,thingVo);
        Classification classificationById = classificationService.getClassificationById(String.valueOf(thing.getClassificationId()));
        thingVo.setClassificationTitle(classificationById.getTitle());
        return new APIResponse(ResponeCode.SUCCESS, "查询成功", thingVo);
    }
